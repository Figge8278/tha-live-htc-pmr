...
export const DRIVE_FOLDERS = {
  client: '01 - Client PMR Report', working: '02 - THA Snapshot Working Files', backup: '99 - Backup & Emergency Restore',
  photos: 'Photos', roomOverview: '01 - Room Overview', findingEvidence: '02 - Finding Evidence',
  clientSubmitted: '03 - Client Submitted', internalReference: '04 - Internal THA Reference'
};
const RENAMES = new Map([['Photos', DRIVE_FOLDERS.working], ['Secondary Editable Copies', DRIVE_FOLDERS.working], ['Backup Data', DRIVE_FOLDERS.backup]]);
...
export function createDrivePackageContext(originalFetch) {
  ...
  function packageIdForParent(parentId = '') {
    ...
    if ([DRIVE_FOLDERS.client, DRIVE_FOLDERS.working, DRIVE_FOLDERS.backup].includes(name)) return parentById.get(parentId) || '';
  ...
  async function ensureFolders(auth, packageId) {
    ...
    const photos = await findOrCreateFolder(auth, DRIVE_FOLDERS.photos, working);
    const value = {
      client, working, backup, photos,
  ...
  async function trackSearch(response, url) {
    try {
      ...
      if (folder?.id && name && parent) { nameById.set(folder.id, name); parentById.set(folder.id, parent); if ([DRIVE_FOLDERS.client, DRIVE_FOLDERS.working, DRIVE_FOLDERS.backup].includes(name)) lastPackageId = parent; }
    } catch {}
  }
  async function trackCreate(response, metadata) {
    ...
    if ([DRIVE_FOLDERS.client, DRIVE_FOLDERS.working, DRIVE_FOLDERS.backup].includes(metadata.name)) lastPackageId = parent;
  }
  ...
  function workingName(name = '') {
    if (/intake summary/i.test(name)) return 'Intake Summary';
    if (/htc checklist/i.test(name)) return 'HTC Checklist';
    if (/photo index/i.test(name)) return 'Photo Index';
