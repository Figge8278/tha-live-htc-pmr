...
function buildPmrReportHtml(payload, photoEntries = []) {
  ...
  const roomSections = Object.entries(roomGroups).map(([room, items]) => `<section class="subsection"><h3>${htmlEscape(room)} <span>${items.length} PMR item${items.length === 1 ? '' : 's'}</span></h3><div class="packet-lines">${items.map(r => visualLine(r, { prefix: room })).join('')}</div></section>`).join('') || '<p><span class="not-recorded">No PMR findings recorded by room.</span></p>';
