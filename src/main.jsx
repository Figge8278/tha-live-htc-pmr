    ...
    <section className={`walkthroughControlsPanel noPrint ${controlsCollapsed ? 'collapsed' : 'expanded'} ${controlsNeedsAttention ? 'needsAttention' : ''}`} aria-label="Walkthrough Control Panel">
      ...
      {!controlsCollapsed && <div className="walkthroughControlsBody">
        ...
        <section className="controlGroup clientCard homeownerOutputCard" aria-label="Homeowner Output">
          ...
          <div className="pmrPrintActions" aria-label="PMR homeowner deliverable actions"><button type="button" onClick={openStyledPMRPreview}><FileText size={16}/> Preview PMR Report</button><button type="button" className="finalPrintButton" onClick={downloadStyledPMR}><Download size={16}/> Download PMR Report</button><button type="button" onClick={()=>window.print()}><Printer size={16}/> Print / Save Draft PMR</button><button type="button" onClick={printFinalPMR}><Printer size={16}/> Print Final PMR</button></div>
        </section>
...
function PMR({client, intake, pmr, counts, quickHits, passCareOutlook = [], passCareCandidates = passCareOutlook, passReview = {}, onPassReviewChange = () => {}, unreviewedIntakeRows = [], reviewedIntakeNotes = []}) {
  ...
  return <main className="pmr">
    ...
    <section className="pmrBlock compactFindings"><h2><AlertTriangle/> PMR Findings — Quick Action List</h2><p className="lede">Compact homeowner-facing rows. Longer observations, rationale, photos, and notes are in the Detail Appendix below. PASS continued-care items are not included in these PMR counts.</p>{pmr.length ? <div className="compactFindingTable" role="table" aria-label="Compact PMR findings"><div className="compactFindingHeader" role="row"><span>Room</span><span>Item</span><span>Level</span><span>Resource / trade</span><span>Suggested next step</span></div>{pmr.map(r => <div className="compactFindingRow" role="row" key={r.id}><span data-label="Room">{r.roomName || r.room}</span><span data-label="Item"><strong>{r.item}</strong></span><span data-label="Level"><span className={`pill ${priority(r.answer.status).toLowerCase()}`}>{priority(r.answer.status)}</span></span><span data-label="Resource / trade"><TradeIcon trade={r.answer.trade}/> {displayTradeLabel(r.answer.trade)}</span><span data-label="Suggested next step">{compactNextStep(r)}</span></div>)}</div> : <p className="lede">No PMR findings recorded.</p>}</section>
    ...
    <section className="pmrBlock detailAppendix"><h2><FileText size={20}/> Detail Appendix</h2><p className="lede">Reference detail for each PMR finding. This keeps the main report short while preserving what was seen, why it matters, confidence, timing, photos, and notes.</p>{pmr.map(r => {
      const certainty = actionCertaintyCopy(r);
