...
function roomItemBucketLabel(bucket) {
  return ROOM_ITEM_BUCKETS.find(option => option.value === bucket)?.label || 'Watch Item / Worth Watching';
}
...
function App() {
  ...
  const roomCaptureFor = (sectionKey) => ({
    ...
    items: Array.isArray(roomCapture?.[sectionKey]?.items) ? roomCapture[sectionKey].items : []
  });
  ...
  const saveRoomItem = (sectionKey) => {
    const title = roomItemDraft.title.trim();
    if (!title) return;
    const current = roomCaptureFor(sectionKey);
    updateRoomCapture(sectionKey, {
      items: [
        ...current.items,
        {
          ...
          isDiscovery: Boolean(roomItemDraft.isDiscovery),
  ...
  return <div className="app">
    ...
    {view === 'form' && <main className="grid">
      ...
      <section className="formPanel">
        <h1>{rooms.find(r=>r.key===activeRoom)?.label || activeRoom} HTC</h1><div className="roomCaptureShell"><div className="roomCaptureTop"><label>Overall Room Status<select value={roomCaptureFor(activeRoom).status} onChange={e=>updateRoomCapture(activeRoom,{status:e.target.value})}>{ROOM_STATUS_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label><button type="button" onClick={()=>setRoomItemFormOpen(true)}>Add Item</button></div><span className="roomCaptureHelp">Add anything that needs tracking beyond ‘looks good.’</span><label className="notes">Room Note / Voice Transcript<textarea value={roomCaptureFor(activeRoom).note} onChange={e=>updateRoomCapture(activeRoom,{note:e.target.value})} placeholder="Capture room-level context, voice transcript, or summary notes for this space."/></label><div className="roomPhotoBox"><div className="photoBox"><Camera size={18}/><strong>Room Overview Photos:</strong><label className="uploadInline"><Upload size={16}/> Add Room Overview Photo<input type="file" accept="image/*" multiple onChange={e=>{addRoomPhotos(activeRoom, e.target.files); e.target.value='';}}/></label><span>{photoSummary(roomCaptureFor(activeRoom).photos, { emptyText: 'No room overview photos attached yet', labels: ROOM_PHOTO_LABELS })}</span></div>{roomCaptureFor(activeRoom).photos.length > 0 && <div className="thumbGrid roomThumbGrid">{roomCaptureFor(activeRoom).photos.map(photo => <div className="thumbCard" key={photo.id}><div className="thumb">{photo.dataUrl ? <img src={photo.dataUrl} alt={`Overview for ${rooms.find(room=>room.key===activeRoom)?.label || activeRoom}`}/> : <Image size={24}/>}</div><span>Overview</span><span title={photo.name}>{photo.name}</span><button onClick={()=>removeRoomPhoto(activeRoom, photo.id)} aria-label="Remove room overview photo"><X size={14}/></button></div>)}</div>}</div><div className="smartRoomPrompt"><h3>Smart Room Prompt</h3><div className="smartRoomGrid">{SMART_ROOM_PROMPTS.map(group => <p key={group.group}><strong>{group.group}:</strong> {group.prompt}</p>)}</div></div><div className="roomItemsPlaceholder"><h3>Items list for this room</h3>{roomItemFormOpen && <div className="roomItemForm"><label>Item title<input value={roomItemDraft.title} onChange={e=>setRoomItemDraft(draft=>({...draft,title:e.target.value}))} placeholder="What needs tracking in this room?"/></label><label>Item bucket/type<select value={roomItemDraft.bucket} onChange={e=>setRoomItemDraft(draft=>({...draft,bucket:e.target.value}))}>{ROOM_ITEM_BUCKETS.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="discoveryToggle"><input type="checkbox" checked={roomItemDraft.isDiscovery} onChange={e=>setRoomItemDraft(draft=>({...draft,isDiscovery:e.target.checked}))}/><span><strong>Discovery</strong><small>Unexpected, hidden, unusual, or out of the ordinary.</small></span></label><label className="notes">Notes<textarea value={roomItemDraft.notes} onChange={e=>setRoomItemDraft(draft=>({...draft,notes:e.target.value}))} placeholder="Optional room-level notes for this item."/></label><div className="roomItemActions"><button type="button" onClick={()=>saveRoomItem(activeRoom)} disabled={!roomItemDraft.title.trim()}>Save</button><button type="button" onClick={()=>{setRoomItemFormOpen(false); setRoomItemDraft(EMPTY_ROOM_ITEM_DRAFT);}}>Cancel</button></div></div>}{roomCaptureFor(activeRoom).items.length > 0 ? <ul className="roomItemList">{roomCaptureFor(activeRoom).items.map(item=><li key={item.id}><div><strong>{item.title}</strong><span>{roomItemBucketLabel(item.bucket)}{item.isDiscovery ? ' · Discovery' : ''}</span>{item.notes && <p>{item.notes}</p>}</div><button type="button" onClick={()=>removeRoomItem(activeRoom,item.id)} aria-label="Remove room-level item"><X size={14}/></button></li>)}</ul> : <p>No room-level items yet for this room.</p>}</div></div><p className="lede">Fuller data capture: status, action certainty, suggested trade, time, notes, and photo references.</p>
