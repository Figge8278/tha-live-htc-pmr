        ...
        <div className="roomCaptureShell">
          <div className={`roomOverviewCard ${currentRoomRailClasses}`}>
            ...
            {isRoomOverviewExpanded && <div className="roomOverviewBody">
              <label className={`roomOverviewField statusControlField ${roomStatusCueClass} ${roomStatusPillClass(currentRoomCapture.status)}`}>Overall Room Status<select value={currentRoomCapture.status} onChange={e=>updateRoomCapture(activeRoom,{status:e.target.value})}>{ROOM_STATUS_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label>
