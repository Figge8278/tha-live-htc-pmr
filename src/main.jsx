...
function reportShell(title, client = {}, body, walkthroughName = '') {
  return `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>${htmlEscape(title)}</title><style>
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--cream);color:var(--ink);font-family:Inter,Segoe UI,Arial,sans-serif;line-height:1.5} main{max-width:1120px;margin:0 auto;padding:22px}
    ...
    .time-guide,.action-guide{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}.time-guide-item,.action-guide-item{display:flex;align-items:flex-start;gap:10px;border:1px solid #d6e2e8;border-radius:14px;background:#f6f9fb;color:var(--navy);padding:12px}.time-guide-item span{font-size:22px}.time-guide-item.future{background:#f5f2fb;border-color:#ddd3ee;color:#4c3a72}.action-guide-item.clearPath{background:#e7f5f5;border-color:#a8d8d8;color:#165f66}.action-guide-item.likelyPath{background:#fff7df;border-color:#ead18b;color:#765714}.action-guide-item.needsDiscovery{background:#f1f4f6;border-color:#ccd4dc;color:#46515b}
        .card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:18px;margin:16px 0;box-shadow:0 8px 22px var(--shadow)}.lede{font-size:16px;color:#40505f;margin-top:0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.stat{background:var(--soft);border-radius:14px;padding:14px;text-align:center}.stat strong{font-size:30px;color:var(--navy);display:block}.pill{display:inline-block;border-radius:999px;background:var(--soft);padding:4px 9px;font-weight:800}.high{background:var(--red)}.medium{background:var(--yellow)}.low{background:var(--green)}.badge-line{display:flex;flex-wrap:wrap;gap:6px;margin:7px 0 10px}.note-card{border:1px solid var(--line);border-radius:14px;padding:14px;margin:12px 0;background:#fffdf8}.detail-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}.detail{background:#fbf7ef;border:1px solid #eadbc2;border-radius:12px;padding:10px}.not-recorded{color:var(--muted);font-style:italic}.small{font-size:12px;color:var(--muted)}
...
function buildPmrReportHtml(payload, photoEntries = []) {
  ...
  const body = `<section class="card"><h2>Report Header</h2><p class="lede">This PMR is the primary pre-client-review report for the working session. PMR findings below are based on reviewed walkthrough observations from the HTC field documentation, with homeowner Intake context used to guide what was checked.</p></section>
    ...
    <section class="card"><h2>PASS — Continued Home Care</h2><p class="lede">PASS items are recurring care reminders for annual or seasonal ownership support. They are not urgent findings unless the same item is separately included in the PMR action plan.</p><ul>${pass.map(r=>`<li><strong>${htmlEscape(r.roomName || r.room)} — ${htmlEscape(r.item)}</strong>: ${htmlEscape(r.answer.passCadence || r.frequency || 'As Needed')} · ${htmlEscape(r.answer.passResource || displayTradeLabel(r.answer.trade))}${r.answer.passNote ? ` · ${htmlEscape(r.answer.passNote)}` : ''}</li>`).join('') || '<li><span class="not-recorded">No PASS candidates recorded.</span></li>'}</ul></section>
