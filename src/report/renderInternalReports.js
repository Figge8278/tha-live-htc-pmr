...
function tone(value = '') {
  ...
  if (/selected|scheduled|completed|recorded|good/i.test(value)) return 'green';
  if (/deferred|not this year/i.test(value)) return 'purple';
  return 'gray';
}
...
function careCard(item = {}) {
  ...
  const status = text(fields.followUpStatus, decision === 'declined' ? 'Deferred / Not this year' : decision);
  const cardTone = tone(status);
  return `<article class="card ${cardTone}"><div class="cardHead"><div><h3>${esc(fields.careItem || fields.careTopic || item.careItemId)}</h3><p>${esc(trade(fields.resource || fields.trade))}</p></div><span class="pill ${cardTone}">${esc(status)}</span></div><div class="detailGrid"><div class="detail"><span>Cadence</span><strong>${rich(fields.cadence,'As Needed')}</strong></div><div class="detail"><span>Last completed</span><strong>${rich(fields.lastCompletedDisplay || fields.lastCompletedDate,'Unknown — establish baseline')}</strong></div><div class="detail"><span>Next window / reminder</span><strong>${rich(fields.deferredReminderDate || fields.nextSuggestedWindow || fields.targetWindow || fields.suggestedWindow,'Next normal care window')}</strong></div><div class="detail"><span>PMCP decision</span><strong>${esc(decision)}</strong></div><div class="detail wide"><span>Reason</span><strong>${rich(fields.reason || fields.passNote,'Routine continued-care planning item.')}</strong></div>${fields.internalNote ? `<div class="detail wide"><span>Internal THA note</span><strong>${rich(fields.internalNote)}</strong></div>` : ''}</div></article>`;
}
...
export function renderOfficeActionPlanHtml(snapshot = {}) {
  ...
  const body = `<section class="section"><h2>THA Office Dashboard</h2><div class="stats"><div class="stat"><strong>${findings.length}</strong><span>Client PMR findings</span></div><div class="stat"><strong>${actions.filter(item=>item.selected).length}</strong><span>Selected THA actions</span></div><div class="stat"><strong>${care.filter(item=>item.reporting?.pmcpDecision==='selected').length}</strong><span>Selected PMCP items</span></div><div class="stat"><strong>${care.filter(item=>item.reporting?.pmcpDecision==='declined').length}</strong><span>Deferred / not this year</span></div></div></section><section class="section"><h2>THA Action Queue</h2><div class="tableWrap"><table><thead><tr><th>Source item</th><th>Action type</th><th>Status</th><th>Internal note</th></tr></thead><tbody>${actionRows || '<tr><td colspan="4">No THA actions selected.</td></tr>'}</tbody></table></div></section><section class="section"><h2>PMR Findings Requiring Coordination</h2>${findings.map(findingCard).join('') || '<p class="empty">No client PMR findings.</p>'}</section><section class="section"><h2>PMCP / Deferred Care</h2>${care.map(careCard).join('') || '<p class="empty">No continued-care records.</p>'}</section>`;
