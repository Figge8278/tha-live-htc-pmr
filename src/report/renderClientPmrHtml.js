...
function referencesHtml(references = []) {
  ...
  return `<details class="referenceBox" open><summary><strong>Important Need-to-Know Home References</strong><span>${recorded}/${references.length} recorded · ${acknowledged}/${required.length} required acknowledged</span></summary><p class="referenceNotice">Orange reference fields must be acknowledged before homeowner delivery. “Not applicable” is a valid acknowledgement when a system is not present.</p><div class="referenceGrid">${references.map(item => `<div class="${item.required ? `requiredReference ${item.acknowledged ? 'acknowledged' : 'missing'}` : 'optionalReference'}"><span>${escapeHtml(item.label)}</span><strong>${richText(item.value, item.status || (item.required ? 'Not acknowledged yet' : 'Not recorded yet'))}</strong>${item.required ? `<small>${item.acknowledged ? 'Required reference acknowledged' : 'Required before delivery'}</small>` : ''}</div>`).join('')}</div></details>`;
}
