...
function careItemHtml(item = {}, deferred = false) {
  const className = deferred ? 'careItem deferredCareItem' : 'careItem';
  const status = deferred ? `Not this year · reminder ${item.targetWindow || 'next year'}` : item.followUpStatus;
  return `<article class="${className}"><div class="careHead"><div><p>${escapeHtml(item.sourceLabel)}</p><h3>${escapeHtml(item.careItem)}</h3></div><span>${escapeHtml(status)}</span></div><div class="detailGrid"><div><span>Recommended cadence</span><strong>${richText(item.cadence)}</strong></div><div><span>Last completed</span><strong>${richText(item.lastCompleted)}</strong></div><div><span>${deferred ? 'Reminder date / next review' : 'Next suggested window'}</span><strong>${richText(item.targetWindow)}</strong></div><div><span>Resource</span><strong>${richText(item.resource)}</strong></div><div class="wide"><span>${deferred ? 'Why it remains on the reminder list' : 'Why this is included'}</span><strong>${richText(item.reason)}</strong></div></div></article>`;
}
export function renderClientPmrHtml(model = {}) {
  ...
  const careItems = model.pmcp?.selected?.length ? model.pmcp.selected.map(item => careItemHtml(item, false)).join('') : '<p class="muted">No continued-care items have been selected for the PMCP yet.</p>';
  const deferredItems = model.pmcp?.deferred?.length ? model.pmcp.deferred.map(item => careItemHtml(item, true)).join('') : '<p class="muted">No items are currently deferred to a future-year reminder.</p>';
