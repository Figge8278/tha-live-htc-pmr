(() => {
  function installStyles() {
    ...
    style.textContent = `
      ...
      /* Walkthrough setup: procedural order 1/2 across, 3/4 beneath. */
  ...
  function arrangeImportPanel(body, setup) {
    ...
    if (heading) heading.textContent = '3. Homeowner Intake';
  ...
  function simplifyControls() {
    document.querySelectorAll('.walkthroughControlsPanel').forEach(panel => {
      ...
      if (subhead) subhead.textContent = panel.classList.contains('collapsed')
        ? 'Open only when you need setup, work-session, homeowner-intake, or record tools.'
        : 'Set up the walkthrough, manage the working session, import homeowner intake context, and save internal records.';
      ...
      renameHeading(panel, 'Send / Import Homeowner Intake', '3. Homeowner Intake');
      renameHeading(panel, 'Homeowner Intake', '3. Homeowner Intake');
      renameHeading(panel, '2. Homeowner Intake', '3. Homeowner Intake');
