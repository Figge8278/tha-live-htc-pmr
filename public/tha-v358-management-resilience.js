(() => {
  const ID = 'tha-v3582-management-resilience';
  if (window[ID]) return;
  window[ID] = true;
  ...
  function run() {
    document.querySelector('.walkthroughControlsPanel')?.classList.add('thaV358NativeControlSource');
    place('.walkthroughSetupCard', '[data-v358-property-host]');
    place('.localWorkCard', '[data-v358-local-host]');
    place('.localBackupRestore', '[data-v358-local-host]');
    place('.intakeImportCard', '[data-v358-existing-host]');
  ...
  new MutationObserver(schedule).observe(document.documentElement, { childList:true, subtree:true, attributes:true, attributeFilter:['data-v358-existing-host','data-v358-records-host','class'] });
})();
