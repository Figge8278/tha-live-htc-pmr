...
function photoKey({ sessionId = '', location = '', ownerKey = '', photo = {}, index = 0 }) {
  return [sessionId || 'walkthrough', location || 'photo', ownerKey || 'owner', photo.id || photo.name || index]
    .map(part => String(part || '').replace(/[^a-z0-9_-]+/gi, '-').slice(0, 80))
    .join(':');
}
...
function writeGuardStatus(status) {
  writeJson(GUARD_STATUS_KEY, { ...status, updatedAt: new Date().toISOString() });
}
...
function getAllPhotoRecords() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      request.onerror = () => {
        db.close?.();
        reject(request.error || new Error('Unable to read photo cache.'));
      };
    } catch (error) {
      reject(error);
    }
...
function restoreSessionsFromCache(sessions, recordsByKey) {
  const clone = JSON.parse(JSON.stringify(sessions || {}));
  let restored = 0;
  ...
  Object.entries(clone || {}).forEach(([sessionId, session]) => {
    ...
    Object.entries(data.roomCapture || {}).forEach(([roomKey, capture]) => {
      if (!Array.isArray(capture?.photos)) return;
      capture.photos = capture.photos.map((photo, index) => restorePhoto(photo, { sessionId, location: 'room', ownerKey: roomKey, index }));
    });
    Object.entries(data.answers || {}).forEach(([answerKey, answer]) => {
      if (!Array.isArray(answer?.photos)) return;
      answer.photos = answer.photos.map((photo, index) => restorePhoto(photo, { sessionId, location: 'item', ownerKey: answerKey, index }));
    });
  });

  function restorePhoto(photo, context) {
    if (!photo || typeof photo !== 'object') return photo;
    if (isPhotoLike(photo)) return photo;
    const key = photo.cacheKey || photo.storageKey || photoKey({ ...context, photo });
    const record = recordsByKey.get(key);
    if (!record?.dataUrl) return photo;
    restored += 1;
    return {
      ...photo,
      cacheKey: key,
      ...
      thumbnailDataUrl: photo.thumbnailDataUrl || record.thumbnailDataUrl || '',
      type: photo.type || record.type || 'image/jpeg',
      photoStorage: 'indexeddb-restored',
      cacheStatus: 'restored',
      needsRestoreForDrive: false
    };
  }
...
async function restoreCachedPhotosBeforeAppLoad() {
  try {
    const raw = originalGetItem.call(localStorage, WALKTHROUGH_SESSIONS_KEY);
    if (!raw) return;
    const sessions = JSON.parse(raw || '{}') || {};
    const needsRestore = JSON.stringify(sessions).includes('"photoStorage":"indexeddb"') || JSON.stringify(sessions).includes('"needsRestoreForDrive":true');
    if (!needsRestore) return;
    writeGuardStatus({ state: 'restoring', message: 'Restoring cached photo payloads before app load.', pending: 0, stripped: 0 });
    const records = await getAllPhotoRecords();
    const recordsByKey = new Map(records.map(record => [record.key, record]));
    const restoredPass = restoreSessionsFromCache(sessions, recordsByKey);
    if (restoredPass.restored) {
      originalSetItem.call(localStorage, WALKTHROUGH_SESSIONS_KEY, JSON.stringify(restoredPass.clone));
    }
    writeGuardStatus({
      state: restoredPass.restored ? 'restored' : 'ready',
      message: restoredPass.restored ? `Restored ${restoredPass.restored} cached photo payload${restoredPass.restored === 1 ? '' : 's'} for Drive/export readiness.` : 'No cached photo payloads needed restoring.',
      pending: 0,
      stripped: 0,
      restored: restoredPass.restored
    });
  } catch (error) {
    writeGuardStatus({ state: 'restore-error', message: error?.message || 'Cached photo restore failed.', pending: 0, stripped: 0 });
  }
}
...
await restoreCachedPhotosBeforeAppLoad();
