(() => {
  const ID = 'tha-v3584-intake-htc-clarity';
  if (window[ID]) return;
  window[ID] = true;
  ...
  const CATEGORY_META = [
    ...
    { slug: 'general-contractor-remodel', label: 'General Contractor / Remodel', icon: '🏗️' },
    ...
    { slug: 'exterior-site-grounds', label: 'Exterior / Site / Grounds', icon: '🏡' },
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .thaV358StepCue{font-size:10px!important;padding:5px 8px!important}
      .thaV358Paths{gap:11px!important}
      .thaV358Path{grid-template-columns:38px minmax(0,1fr) auto!important;gap:3px 10px!important;padding:13px 14px!important;min-height:74px!important}
      .thaV358PathIcon{width:36px!important;height:36px!important;border-radius:11px!important;font-size:19px!important}
      .thaV358Path h3{font-size:16px!important;line-height:1.2!important}
      .thaV358Path p{font-size:10px!important;line-height:1.3!important}
      .thaV358Path>button{font-size:11px!important;padding:9px 12px!important;min-height:38px!important}
      .thaV358PathWorkspaceHeader h3{font-size:18px!important}
      .thaV358PathWorkspaceHeader p{font-size:11px!important}
      .thaV358ExistingChoices button{font-size:11px!important;padding:10px 12px!important;min-height:40px!important}
      ...
      @media(max-width:1050px){
        .thaV358Path h3{font-size:15px!important}
        .thaV358TradeGroup .checklistSummaryRow{padding:9px!important}
      }
      @media(max-width:700px){
        .thaV358BeginHeader{align-items:flex-start!important}
        .thaV358Path{grid-template-columns:36px minmax(0,1fr) auto!important}
        .thaV358Path p{display:block!important}
        .thaV3584IntakeActionRow{align-items:stretch;flex-direction:column}
        .thaV3584IntakeActionRow .copyIntakeEmailButton{width:100%!important;margin-left:0!important;justify-content:center!important}
      }
    `;
  ...
  function categoryForCard(card) {
    return CATEGORY_META.find(item => card.classList.contains(`category-${item.slug}`)) || CATEGORY_META[CATEGORY_META.length - 1];
  }
  ...
  function polishStart(page) {
    ...
    if (kind === 'local') {
      ...
      host?.querySelectorAll('.intakeImportCard,.businessRecordsCard,.thaSnapshotSourcePanel').forEach(node => node.style.removeProperty('display'));
    }
  ...
  function addClientFacingBadge(label, mustAnswer) {
    ...
    refresh();
    if (field && !field.dataset.v3584ClientFacingBound) {
      field.dataset.v3584ClientFacingBound = 'true';
  ...
  function decorateIntake() {
    ...
    page.querySelectorAll('.intakeSubsection label').forEach(label => {
      if (!labelMatches(label, CLIENT_FACING_INTAKE)) return;
      addClientFacingBadge(label, labelMatches(label, REQUIRED_REFERENCES));
    });
  }
  ...
  function decorateHtcClientFacing() {
    ...
    document.querySelectorAll('.roomOverviewBody label.notes').forEach(label => {
      if (/Room Note \/ Voice Transcript/i.test(text(label.textContent))) {
  ...
  function setCategoryIcon(card) {
    const meta = categoryForCard(card);
    card.querySelectorAll('.tradeIcon').forEach(icon => {
      if (text(icon.textContent) !== meta.icon) icon.textContent = meta.icon;
    ...
    card.querySelectorAll('.checklistSummaryMain>span:last-child,.expandedItemHead>div>p').forEach(node => {
      if (/Suggested:/i.test(node.textContent)) node.textContent = node.textContent.replace(/Suggested:/i, 'Likely resource:');
    });
  }

  function groupChecklist(panel) {
    if (!panel) return;
    const directCards = Array.from(panel.children).filter(node => node.classList?.contains('checklistItemCard'));
    const existingCards = Array.from(panel.querySelectorAll(':scope > .thaV3584TradeGroups .checklistItemCard'));
    const cards = [...new Set([...directCards, ...existingCards])];
    ...
    CATEGORY_META.forEach(meta => {
      const matching = cards.filter(card => card.classList.contains(`category-${meta.slug}`));
      let group = host.querySelector(`:scope > [data-v3584-category="${meta.slug}"]`);
      if (!matching.length) {
        group?.remove();
        return;
      }
      if (!group) {
        ...
        group.dataset.v3584Category = meta.slug;
        group.innerHTML = `<header class="thaV3584TradeGroupHeader"><span aria-hidden="true">${meta.icon}</span><div><strong>${meta.label}</strong><small>System/category icon · likely resource remains item-specific</small></div></header><div class="thaV3584TradeGroupBody"></div>`;
      }
      host.append(group);
  ...
  new MutationObserver(schedule).observe(document.documentElement, {
    ...
    subtree: true,
    attributes: true,
    attributeFilter: ['class', 'open', 'hidden', 'data-v358-hidden', 'data-kind', 'data-subkind']
  });
})();
